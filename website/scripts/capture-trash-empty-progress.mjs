 ...
 * safe), finished (what it freed), and stopped (why nothing more went). Captured
...
const RUNNING = {
  job_id: 'empty-1', running: true, started_at: now - 95, finished_at: 0,
  total_bytes: 18_000_000_000, total_sessions: 55_323, freed_bytes: 6_900_000_000, error: '',
}
const DONE = {
  ...RUNNING, running: false, finished_at: now, freed_bytes: 18_000_000_000,
}
const STOPPED = {
  ...DONE, freed_bytes: 4_100_000_000,
  error: 'a staged file is absent from its manifest, so it is the only copy',
}
...
async function main() {
  ...
  await frame(base, browser, 'stopped', { job: STOPPED, list: inventory })

  await browser.close()
  srv.close()
  console.log(`wrote ${OUT}/running.png, ${OUT}/finished.png and ${OUT}/stopped.png`)
}
