 ...
 *
 * Quoted only when a value would not survive the round trip bare: empty, or with
 * whitespace at an end that a re-parse would trim.
 */
export function renderDotenv(pairs: Record<string, string>): string {
  return Object.keys(pairs).sort()
    .map(name => {
      const value = pairs[name]
      return value === '' || value !== value.trim() ? `${name}="${value}"` : `${name}=${value}`
    })
    .join('\n')
}
...
interface TableProps {
   ...
   */
  onSave: (
    change: { set?: Record<string, string>; delete?: string[]; bulk?: string },
    onLanded?: () => void,
  ) => void
  busy: boolean
...
function VariableTable({
  ...
  const applyBulk = () => {
    setError(null)
    onSave({ bulk: bulkText }, () => setBulkOpen(false))
  }
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-col md:flex-row md:flex-wrap md:items-end gap-2 pt-1">
        ...
        <Btn
          type="button"
          disabled={busy}
          onClick={() => { setBulkText(renderDotenv(pairs)); setError(null); setBulkOpen(true) }}
        >
