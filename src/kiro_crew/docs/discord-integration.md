...
- **Slash commands answer privately.** A `/` command reply is ephemeral, so only
  the person who ran it sees it. `!dashboard` is refused outright outside a DM
  rather than relying on that, because a presigned login link is a credential
...
| `!yolo [on\|off\|renew]` | Report or change auto-approve for every tool |
...
| `!dashboard [2h]` | Get a dashboard login link, **DM only** |
...
`!dashboard` mints a presigned login link for the whole dashboard, so it is
refused outside a direct message. An approved thread is readable by every member
who can see it, and a link posted there would hand them your dashboard.
