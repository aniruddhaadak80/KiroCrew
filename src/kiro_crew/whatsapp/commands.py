...
Deliberately NOT ported from Slack, because each needs a capability this
channel does not have or a trust boundary a phone must not cross:
...
- ``/voice``: a spoken reply is an outbound audio upload, and this channel
  declares ``files_outbound=False``.
