    ...
    async def _drive(self, inbound: InboundMessage, verdict: Any) -> None:
        ...
        await drive_turn(
            ChannelTurn(
                ...
                # A non-operator's PROMPT carries none of the operator's memory,
                # lessons, skills or history. Denying their tools does not reach
                # this: the disclosure is in the assembled context, before any
                # tool runs, so an admitted peer could otherwise be answered out
                # of the operator's private material without asking for it.
                minimal_context=not is_operator,
                persist=(
