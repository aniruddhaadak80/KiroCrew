...
class WhatsAppTransport(MessagingTransport):
    ...
    async def receive(self, raw_envelope: Any) -> None:
        ...
        # Media is fetched only now, AFTER the group gate and authorize. Doing it
        # earlier would let any stranger who can message the number trigger an
        # authenticated download on the operator's host, which is a
        # remote-triggered fetch with no authorization behind it.
        temp_paths: list[str] = []
