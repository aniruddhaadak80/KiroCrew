...
async def cleanup_offloaded(paths: list[str]) -> None:
    ...
    Falls back to deleting inline if the loop cannot take the work at all
    (already closed, mid-teardown). Blocking a loop that is finished costs
    nothing, whereas skipping the delete would leave a user's DECRYPTED
    attachment readable on disk — the whole reason these handlers exist.

    Returns normally in that case rather than re-raising: the caller is about to
    ``raise`` the exception it was already handling, and surfacing this one
    instead would replace the real reason the turn ended.
    """
    if not paths:
        return
    try:
