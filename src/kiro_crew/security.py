    ...
    * Per-segment (pass 2) re-evaluation.  Segment splitting exists to isolate a chained
    ...
    for pattern, is_regex in all_patterns:
        if not _deny_pattern_matches(pattern, lower, is_regex):
            continue
        exceptions = _DENY_EXCEPTIONS.get(pattern, [])
        if exceptions and any(fnmatch.fnmatch(lower, e.lower()) for e in exceptions):
            # Mirror the shared path's fail-closed audit contract: an exception is
            # granted only if the grant can be recorded.
            if _emit_deny_exception_event(target, pattern):
                continue
