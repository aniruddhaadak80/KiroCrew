...
_REPLY_COMMANDS = frozenset({"status", "yolo", "dashboard"})
...
class DiscordDispatcher:
    ...
    async def on_interaction(self, itx: "DiscordInteraction") -> None:
        ...
        if data.startswith("opt:"):
            ...
            # a prompt-injected agent offer `!yolo on` as a choice and have one
            # click grant global auto-approve, or `!dashboard` and have it mint a
            # login link. Same rule and same reason as the queue drain, which
    ...
    async def _handle_yolo(self, reply: "ReplyFn", arg: str, user_id: str) -> None:
        """Report or change the global auto-approve grant.

        Drives the SAME process-wide :func:`safety_override` grant as the
        dashboard toggle, Slack's ``/kirocrew yolo`` and Telegram's ``/yolo``, so
        a grant taken here shows up (and expires) everywhere. Reachable only by
        an allow-listed user, because both entry points authorize before dispatch.

        Turning it on does NOT weaken the PreToolUse gate: the sensitive-path
        keystone, the governance ceiling and the deny-list all run ahead of the
        auto-approve ladder, so a hard DENY still wins.

        The three mutators run off-loop: ``activate`` resolves the ad-hoc
        duration through a live config read and each writes a SEL record, so
        calling them inline would put filesystem latency on the event loop and
        stall every other chat and heartbeat task on a slow disk.
        """
        so = safety_override()
        ...
        if action in ("on", "off", "renew"):
            ...
            if action == "on":
                if so.is_active():
                    text = f"🟢 YOLO is already ON ({describe_grant_lifetime()})."
                elif (await asyncio.to_thread(so.activate, "discord")).active:
                    text = (
                        f"🟢 YOLO ON ({describe_grant_lifetime()}) — every tool "
                        f"auto-approves. Denied-by-policy tools are still blocked."
                    )
                else:
                    text = "❌ Couldn't turn YOLO on (audit system unavailable)."
                    outcome = "denied"
            elif action == "off":
                # Unconditional: deactivate() also zeroes the deadline of a grant
                # that already lapsed, which closes the renew grace window so a
                # later "renew" cannot resurrect it, and records the decision
                # either way.
                await asyncio.to_thread(so.deactivate, "discord")
                text = "🔴 YOLO OFF — tools ask for approval again."
            else:
                renewed = (await asyncio.to_thread(so.renew, "discord")).renewed
                text = (
        ...
        await reply(f"YOLO is {status}.\nUsage: `!yolo on | off | renew`")

    async def _handle_dashboard(
        self, reply: "ReplyFn", text: str, user_id: str, thread_id: str
    ) -> None:
        """Mint and send a presigned dashboard login link.

        DM-ONLY. A presigned link is a bearer credential for the whole dashboard,
        and an approved guild thread is readable by every member who can view it,
        so a group request is refused with a pointer to DM rather than relying on
        the reply being ephemeral: the same token-leak policy as Slack's
        always-DM delivery and Telegram's forum refusal.
        """
        ...
        if thread_id:
            await reply(
                "🔒 Dashboard links are only sent in a direct message — " "DM me `!dashboard`."
            )
            return
        ttl_secs = min(parse_dashboard_ttl(text), MAX_SESSION_TTL_SECS)
        try:
            token = generate_token(user_id, ttl_seconds=ttl_secs)
            ...
            if not origin:
                ...
                _host, port = parse_dashboard_url(self.cfg.dashboard.url)
                origin = f"http://localhost:{port}"
            url = f"{origin}/?token={token}"
            # Credential issuance MUST be audited (backend-security-controls),
            # mirroring slack.dashboard_token and telegram.dashboard_token.
            sel().log_api_access(
                caller=user_id,
                operation="discord.dashboard_token",
                outcome="ok",
                source="discord",
                resources=f"ttl={ttl_secs}",
            )
            await reply(f"🔗 Dashboard link (valid {format_ttl(ttl_secs)}):\n{url}")
        except Exception as exc:
            logger.warning("discord !dashboard: token generation failed", exc_info=True)
            try:
                sel().log_api_access(
                    caller=user_id,
                    operation="discord.dashboard_token",
                ...
                pass
            await reply(f"⚠️ Could not generate dashboard link: {exc}")
    ...
    #: Commands that act on the OPERATOR's authority rather than the caller's
    #: conversation: one grants gateway-wide auto-approve, the other mints a
    #: dashboard login credential. ``discord.allowed_user_ids`` answers "may drive
    #: the agent", which is not the same claim, so these two need the owner gate
    #: `!sessions` already uses.
    _OWNER_ONLY_COMMANDS = frozenset({"yolo", "dashboard"})
    ...
    async def _run_reply_command(
        ...
        *,
        ...
        """
        if cmd in self._OWNER_ONLY_COMMANDS and not self._session_resume.is_owner(user_id):
            # `owner_id` is the SOLE allow-listed user, empty when there are
            # several, so an install with two approved users has no owner and both
            # commands refuse rather than letting either one act as one. Refusing
            # is the safe direction: the alternative is any approved member
            # granting auto-approve for every tool the gateway will run, or
            # minting a login link to the whole dashboard.
            sel().log_api_access(
                caller=user_id,
                operation=f"discord.{cmd}",
                outcome="denied",
                source="discord",
                resources="not_owner",
            )
            await reply(
                f"🔒 `!{cmd}` is owner-only. It is available when exactly one "
                f"Discord user ID is allow-listed, so there is an unambiguous "
                f"owner to attribute it to."
            )
            return
        if cmd == "status":
            await self._handle_status(reply)
        elif cmd == "yolo":
            await self._handle_yolo(reply, parse_command_argument(text), user_id)
        elif cmd == "dashboard":
            await self._handle_dashboard(reply, text, user_id, thread_id)
