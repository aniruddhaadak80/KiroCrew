...
def _learn(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if action == "add":
            ...
            result = vs.write_lesson_ex(rule, category, negative)
            neg = f" ({negative})" if negative else ""
            if result.wrote:
                print(f"Saved: {rule}{neg} [{category}]")
            elif result.outcome is LessonWriteOutcome.UNCHANGED:
                # The submitted category is deliberately NOT echoed here. ``unchanged``
                # means nothing was written, and the store keeps the stored category
                # and NOT-clause rather than rewriting them on a re-submit -- so
                # printing what was just typed would show a value the store may not
                # hold. `learn list` shows what is actually stored.
                print(
                ...
                # REFUSED -- and any outcome a later change adds, because an outcome
                # that stores something has to join ``wrote`` above to be useful. A
