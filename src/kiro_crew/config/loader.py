...
def _validate_telegram_activation(value: str) -> str:
    """*value* if Telegram can express it, else ``always``.

    Degrades to ``always`` rather than to ``mention``, because an unreadable value
    must not quietly make a working bot stop answering — the same reasoning as
    ``_normalize_acp_backend`` degrading to the floor with a logged reason instead
    of raising. A typo costs the operator the narrowing they asked for, which they
    can see; it does not cost them the channel, which they cannot.
    """
    if value in TELEGRAM_ACTIVATIONS:
        return value
    logger.warning(
        "telegram.forum_activation=%r is not one of %s; using %r.",
        value,
