...
async def api_variables(request: web.Request) -> web.Response:
    ...
    if raw_bulk is not None:
        ...
        # Expressed as a per-key diff rather than a whole-scope replace. Deletion
        # stays "absence" with no second verb, and two operators editing different
        # keys still cannot lose each other's edits — the property the per-key PUT
        # was built for, which a wholesale overwrite would give back.
