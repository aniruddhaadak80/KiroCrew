...
async def _resolve_ledger_key(
    ...
    """
    # Deferred: handlers.cron imports learn/vector-store machinery this module
    # never needs on its own load path.
...
async def api_session_ledger_record(request: web.Request) -> web.Response:
    ...
    if artifacts is not None and not all(
        ...
        return web.json_response({"error": "artifacts must map strings to strings"}, status=400)
    try:
        ...
        return web.json_response({"error": "ledger write failed; try again"}, status=503)
