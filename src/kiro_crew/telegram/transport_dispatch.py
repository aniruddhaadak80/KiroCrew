...
class TelegramDispatcher:
    ...
    def _addresses_this_bot(self, msg: InboundMessage) -> bool:
        ...
        * the message replies to one sent by this bot, matched on ``bot_id``.
        ...
        if not handle:
            return False
        return f"@{handle}".lower() in (msg.text or "").lower()
    ...
    async def _handle_title(self, route: tuple[str, str], chat_id: int, arg: str) -> None:
        ...
        if self.conv_log is None:
            ...
            return
        try:
            await asyncio.to_thread(self.conv_log.set_title, self._session_key(route), title)
        except Exception:
    ...
    async def _handle_spawn(
        ...
        if self.subagent_manager is None:
            ...
            return
        reply = spawn_task_reply(arg, self.subagent_manager, self._session_key(route))
        if reply is None:
    ...
    async def _handle_task(
        ...
        *,
        ...
        if self.task_runner is None:
            ...
            return
        reply = await task_arg_reply(arg, self.task_runner, session_key=self._session_key(route))
        if reply is None:
            await self._reply(
                chat_id,
    ...
    async def on_callback(self, cb: "TelegramCallback") -> None:
        ...
        if data.startswith("a:"):
            ...
            pending = TelegramApprovalDecider.is_pending(key)
