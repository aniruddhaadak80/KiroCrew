...
class WeComRenderer(Renderer):
    ...
    async def on_done(self, stop_reason: str = "") -> None:
        ...
        if not (self._stream_ok and await self._send_final_chunk(head)):
            # Stream never started or died mid-turn -- one-shot POST. The
            # response_url is single-use, so it can carry the head only.
    ...
    def _roll_if_sealed(self) -> None:
        ...
        if not sealed and not aged:
            return
        # An AGED roll happens before any refusal, so everything already sent is
        # known-delivered and the continuation resumes exactly after it. Only a
        # SEALED roll has to be conservative about the newest frame.
        self._carried = self._prev_sent_abs if sealed else self._sent_abs
    ...
    async def _push(self, *, force: bool) -> None:
        ...
        if not body and self._reasoning:
            ...
            reasoning = f"<think>{''.join(self._reasoning)}</think>"
            self._stream_ok = await self._client.send_stream(
