...
class WeComDispatcher:
    ...
    async def handle_message(self, inbound: "WeComInbound") -> None:
        ...
        if cmd == "new":
            ...
            await self.client.send_reply(inbound.response_url, _NEW_SESSION)
            return
        if cmd == "compact":
        ...
        if cmd in ("link", "unlink"):
            ...
            await self.client.send_reply(inbound.response_url, _LINK_UNAVAILABLE)
            return
        if cmd == "help":
            await self.client.send_reply(inbound.response_url, build_help())
            return
        if cmd == "stop":
    ...
    async def _handle_stop(self, inbound: "WeComInbound") -> None:
        ...
        await self.client.send_reply(inbound.response_url, ack)
    ...
    async def _handle_busy(self, inbound: Any, session_key: str) -> None:
        ...
        if steered:
            await self.client.send_reply(inbound.response_url, _STEER_MERGED)
        else:
            await self.client.send_reply(inbound.response_url, _STEER_UNAVAILABLE)
    ...
    async def _handle_compact(self, inbound: "WeComInbound") -> None:
        ...
        if not await self.sessions.try_acquire(session_key):
            if self.sessions.has_session(session_key):
                await self.client.send_reply(inbound.response_url, _COMPACT_BUSY)
            else:
                await self.client.send_reply(inbound.response_url, _COMPACT_NOTHING)
            return
        try:
            provider = self.sessions.get_provider(session_key)
            if provider is None:
            ...
            await provider.compact()
            await provider.wait_for_compaction()
            await self.client.send_reply(inbound.response_url, _COMPACT_DONE)
        except Exception:
            logger.exception("WeCom /compact failed for %s", session_key)
            await self.client.send_reply(inbound.response_url, _COMPACT_FAILED)
        finally:
