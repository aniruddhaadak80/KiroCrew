...
class SlackRenderer(Renderer):
    ...
    async def _withhold_refs(self, text: str) -> str:
        ...
        cut = spans[0][0] if spans else len(self._ref_hold)
    ...
    async def _release_refs(self) -> str:
        ...
        return await asyncio.to_thread(hide_local_refs, held)
    ...
    def _rejection_notes(self, rejections: list[Rejection]) -> str:
        ...
        # scanned like any other outbound text before it can be posted.
        note, _ = redact_exfiltration_urls(note)
        note, _ = redact_credentials(note)
        return note
