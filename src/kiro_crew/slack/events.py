...
from kiro_crew.safety_override import grant_declared_yolo, safety_override, yolo_policy_permits
...
async def init_socket_mode(orch: GatewayOrchestrator, seen: SeenCache) -> None:
    ...
    # Offload the YOLO grant walk (profiles-dir scan) to a thread.
    # This sits between two early returns (owner check, enterprise validation)
    # so it must stay inside this function to preserve the return ordering.
    if orch._cfg.agent.dangerously_skip_permissions:
        set_yolo_mode(True)
        await asyncio.to_thread(grant_declared_yolo)

    # Offload the enterprise auth.test network call to a thread.
