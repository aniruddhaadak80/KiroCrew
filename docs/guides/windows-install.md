...
- **Full-window assisted installer, per user by default** — the themed setup
  ...
  app theme, keeps every functional label and control native (including all 26
  bundled installer languages), and replays all eight Kiro characters with the
  opening animation's staggered entrance, continuous bob, and interleaved blink
  throughout setup.
  New installs default to the current user, a desktop shortcut and
  starting Kiro Crew with Windows; both checkboxes can be cleared before
  installing. A current-user install needs no UAC prompt. Choosing "all users"
  switches the destination to Program Files and requests elevation. The
