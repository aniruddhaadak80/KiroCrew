...
shutdown takes. The delete survives a second cancellation because the work is
already queued on the executor, which still drains through
`shutdown_default_executor`. If the loop cannot take the work at all (already
closed) it deletes inline and returns normally — blocking a finished loop costs
nothing, skipping the delete leaves a user's DECRYPTED attachment on disk, and
re-raising would replace the real reason the turn ended.
...
`Content-Length`. `voice` is excluded from the download path on purpose: WeCom
