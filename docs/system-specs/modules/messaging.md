...
`!yolo`, `!status`, `!dashboard`, `!link`, `!unlink`, `!stop`, `!help`,
`!sessions`, `!queue`, `!steer`) because Discord's client swallows a bare `/`
...
- **An explicit `job.channel` still wins.** The stand-down requires
  `not job.channel`, so a destination the user pinned always keeps its Slack
  delivery. The channel leg is not conditioned on it, so a pinned job whose origin
  is also a channel does reach both surfaces: that is the user asking for the
  pinned one by name, not the default.
...
`status` / `yolo` / `dashboard` are single-reply commands whose handlers take a
`ReplyFn` sink, so the text and slash surfaces share one body and differ only in
where the reply goes. `status` reports `Stats().summary()`, the same source
Slack's `/kirocrew status` uses, so the two channels cannot report different
numbers for one gateway. `yolo` drives the process-wide `safety_override` grant
the dashboard and every other channel share, off-loop, SEL-audited. `dashboard`
is DM-only and refuses in a thread: a presigned link is a bearer credential for
the whole dashboard, matching Slack's always-DM delivery and Telegram's forum
refusal. `model` is button-only and does NOT take a sink: its buttons must live
