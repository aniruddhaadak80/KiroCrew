...
`MAX_MEDIA_BYTES` is measured against the bytes that ARRIVE, never against the
declared `fileLength`, and the write to the shared temp path is offloaded because
