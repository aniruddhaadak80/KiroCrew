...
- Windows target: assisted NSIS. Its borderless, theme-aware 1280×860 surface
  puts native localized controls over light/dark frosted-glass artwork, with
  ...
  characters with a one-time staggered entrance, continuous bob, and interleaved
  blink through compact 24-bit bitmap crops, without embedding a browser or
  WebView.
...
6. **Package** — in `website/electron/`, runs electron-builder to produce the
   ...
   uses two full light/dark surfaces plus small per-ghost animation crops rather
   than duplicating the entire 1280×860 frame for every animation step.
