...
def test_events_tail_bounded():
    ...
    assert len(sl.read_events(key)) == sl._MAX_EVENT_TAIL
...
def _open_route(monkeypatch):
    ...
    monkeypatch.setattr("kiro_crew.dashboard.handlers.cron._recognize_session", _recognized)
...
async def test_route_refuses_unrecognized_session(monkeypatch):
    ...
    monkeypatch.setattr("kiro_crew.dashboard.handlers.cron._recognize_session", _refused)
