...
def test_an_unreadable_activation_degrades_to_always_not_to_silence(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A typo must not make a working forum bot stop answering.

    ``mention`` would be the deny-by-default choice, and it is wrong here: the
    operator can SEE that they lost the narrowing they asked for, but they cannot
    see a bot that has quietly stopped responding to anything.
    """
    monkeypatch.setenv("KIROCREW_HOME", str(tmp_path))
    (tmp_path / "config.json").write_text(
