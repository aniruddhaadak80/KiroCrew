...
``write_lesson`` returns a bare ``bool``, and its ``False`` meant several unrelated
things at once: validation refused the value, a dedup rule claimed the write, the
submit was a no-op, or a bare re-submit deliberately kept a stored NOT-clause. The
...
bool stays a projection of it (so the callers that only branch on success are
unaffected), and that the three surfaces a human or a model reads -- the CLI, the
``/api/lessons`` response and the ``learn_add`` tool result -- stop saying "Saved" for
a write that stored nothing.
"""
...
class TestWriteLessonExOutcomes:
    ...
    def test_attaching_a_clause_to_a_stored_rule_is_enriched(self, tmp_path):
        store = _store(tmp_path)
        try:
            store.write_lesson_ex("prefer ruff over flake8", "tool")
            result = store.write_lesson_ex("prefer ruff over flake8", "tool", "not for typing")
    ...
    def test_identical_resubmit_is_unchanged_not_a_failure(self, tmp_path):
        ...
        store = _store(tmp_path)
        try:
            store.write_lesson_ex("prefer ruff over flake8", "tool", "not for typing")
            result = store.write_lesson_ex("prefer ruff over flake8", "tool", "not for typing")
    ...
    def test_refused_value_reports_the_reject_code(self, tmp_path):
        ...
        store = _store(tmp_path)
        try:
            result = store.write_lesson_ex("run the deploy script", "tool", _INJECTION)
    ...
    def test_inadmissible_scope_is_refused_with_its_own_reason(self, tmp_path):
        store = _store(tmp_path)
        try:
            result = store.write_lesson_ex("gate the repo", "tool", None, repo_scope="/src/pkg")
            assert result.outcome is LessonWriteOutcome.REFUSED
    ...
    def test_substring_covered_rule_is_deduped_not_refused(self, tmp_path):
        ...
        store = _store(tmp_path)
        try:
            store.write_lesson_ex("always run the linter before pushing a branch", "tool")
            result = store.write_lesson_ex("run the linter", "tool")
...
class TestWriteLessonBoolIsAProjection:
    """The bool must keep answering exactly "did this write something".

    The reason the return type was not simply changed: a tuple or a string enum is
    always truthy, so every ``if store.write_lesson(...)`` in the tree would have
    silently become unconditional, and mypy flags only the annotated assignments --
    not the bare ``if``. These pin that the two forms cannot drift.
    """

    @pytest.mark.parametrize(
        "negative,expect_wrote",
        [(None, True), (_INJECTION, False)],
    )
    def test_bool_matches_the_result_for_the_same_input(self, tmp_path, negative, expect_wrote):
        store = _store(tmp_path)
        try:
            assert store.write_lesson("a fresh rule about caching", "tool", negative) is (
                expect_wrote
            )
        finally:
            store.close()

    def test_bool_is_false_for_a_no_op_resubmit(self, tmp_path):
        store = _store(tmp_path)
        try:
            assert store.write_lesson("prefer ruff over flake8", "tool") is True
            assert store.write_lesson("prefer ruff over flake8", "tool") is False
        finally:
            store.close()
...
class TestLessonsRouteReportsTheOutcome:
    ...
    async def _post(self, result):
        ...
        vs.write_lesson_ex.return_value = result
        with (
    ...
    async def test_a_declining_outcome_still_refreshes(self, result):
        ...
        ``write_lesson_ex``'s second pass DELETES a row it supersedes and keeps
