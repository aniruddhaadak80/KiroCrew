...
# ── 1. The dashboard-link TTL floor (Discord + Telegram) ────────────────────


class TestDashboardTtlFloor:
    """A requested lifetime of zero must never become a zero-second credential."""

    def test_zero_hours_is_raised_to_the_floor_on_both_channels(self) -> None:
        assert dcmd.parse_dashboard_ttl("!dashboard 0h") == dcmd.MIN_DASHBOARD_TTL_SECS
        assert tcmd.parse_dashboard_ttl("/kirocrew dashboard 0h") == tcmd.MIN_DASHBOARD_TTL_SECS

    def test_zero_minutes_is_raised_too(self) -> None:
        assert dcmd.parse_dashboard_ttl("!dashboard 0m") == dcmd.MIN_DASHBOARD_TTL_SECS
        assert tcmd.parse_dashboard_ttl("/kirocrew dashboard 0m") == tcmd.MIN_DASHBOARD_TTL_SECS

    def test_the_floor_is_positive_and_identical_on_both_channels(self) -> None:
        """The two channels mint the same credential, so one floor, not two.

        A floor on one channel and not the other is the same hole with a
        different entry point, which is why this is pinned rather than left to
        each module.
        """
        assert dcmd.MIN_DASHBOARD_TTL_SECS == tcmd.MIN_DASHBOARD_TTL_SECS
        assert dcmd.MIN_DASHBOARD_TTL_SECS > 0

    def test_durations_a_user_can_type_are_untouched(self) -> None:
        assert dcmd.parse_dashboard_ttl("!dashboard 2h") == 7200
        assert dcmd.parse_dashboard_ttl("!dashboard 30m") == 1800
        assert dcmd.parse_dashboard_ttl("!dashboard 1m") == 60
        assert tcmd.parse_dashboard_ttl("/kirocrew dashboard 2h") == 7200
        assert tcmd.parse_dashboard_ttl("/kirocrew dashboard 30m") == 1800

    def test_absent_or_unparseable_durations_still_default_to_an_hour(self) -> None:
        assert dcmd.parse_dashboard_ttl("!dashboard") == 3600
        assert dcmd.parse_dashboard_ttl("!dashboard soon") == 3600
        assert tcmd.parse_dashboard_ttl("/kirocrew dashboard") == 3600
        assert tcmd.parse_dashboard_ttl("/kirocrew dashboard soon") == 3600

    def test_the_reply_can_never_render_a_zero_length_lifetime(self) -> None:
        """``format_ttl`` is what the reply shows, so the floor must survive it."""
        assert dcmd.format_ttl(dcmd.parse_dashboard_ttl("!dashboard 0h")) == "1m"
        assert tcmd.format_ttl(tcmd.parse_dashboard_ttl("/kirocrew dashboard 0h")) == "1m"


@pytest.mark.asyncio
async def test_discord_dashboard_command_mints_a_usable_link_and_reports_its_life(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """``!dashboard 0h`` gets a working link whose stated lifetime is the real one."""
    minted: list[int] = []

    def _fake_token(user_id: str, ttl_seconds: int = 0) -> str:
        minted.append(ttl_seconds)
        return "TOKEN"

    monkeypatch.setattr("kiro_crew.dashboard.token_auth.generate_token", _fake_token)

    dispatcher = _dispatcher()[0]
    replies: list[str] = []

    async def _reply(text: str) -> None:
        replies.append(text)

    await dispatcher._handle_dashboard(_reply, "!dashboard 0h", "u1", "")

    assert minted == [dcmd.MIN_DASHBOARD_TTL_SECS]
    assert len(replies) == 1
    assert "valid 1m" in replies[0]
    assert "0m" not in replies[0]
    assert "?token=TOKEN" in replies[0]


# ── 2. An unrecognized ``!command`` (and the prose that must not trip it) ───
...
class TestUnknownCommandUsage:
    ...
    def test_a_mistyped_command_with_arguments_is_caught_too(self) -> None:
        assert dcmd.unknown_command_usage("!dashbord 2h") != ""
...
# ── Discord dispatcher harness (items 2 + 3 end to end) ────────────────────
