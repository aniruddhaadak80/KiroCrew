...
class TestCloneFailureDiagnostics:
    ...
    async def test_owner_designated_failure_has_no_misleading_hint(self, tmp_path):
        ...
        assert err is not None
        assert err["ok"] is False
        # Bare honest failure — the machine slug is the generic `git_clone_failed`
        # (a non-2xx body must carry a `code`, AGENTS.md invariant), NOT the
        # credential-posture `git_clone_failed_no_credentials`, and the human
        # `error` carries no credential-posture hint (the clone kept the ambient
        # identity, so a "withheld credentials" hint would be wrong here).
        assert err["error"] == "git clone failed"
