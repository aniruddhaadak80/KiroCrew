...
class TestLearnCli:
    def test_add_prefers_vector_store(self, capsys: pytest.CaptureFixture[str]) -> None:
        with _LearnHarness() as h:
            h.vs.write_lesson_ex.return_value = LessonWriteResult(LessonWriteOutcome.INSERTED)
            cc._learn(_ns(learn_action="add", rule="do x", category="tool", negative="not y"))
        h.vs.write_lesson_ex.assert_called_once_with("do x", "tool", "not y")
    ...
    def test_add_does_not_write_jsonl_when_the_store_declines(
        ...
        """
        with _LearnHarness() as h:
            h.vs.write_lesson_ex.return_value = LessonWriteResult(LessonWriteOutcome.UNCHANGED)
    ...
    def test_add_refusal_exits_non_zero_without_writing_jsonl(
        ...
        """
        with _LearnHarness() as h:
            h.vs.write_lesson_ex.return_value = LessonWriteResult(
