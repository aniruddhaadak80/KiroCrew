...
def _trust_press() -> SimpleNamespace:
    """A Trust button press ("a:<rid>:t") from the allow-listed owner's DM."""
...
class TestSessionTrust:
    ...
    async def test_the_prompt_offers_trust_beside_approve_and_deny(self) -> None:
        ...
        assert data == ["a:r1:1", "a:r1:0", "a:r1:t"]
    ...
    async def test_a_trust_press_grants_before_it_resolves(self) -> None:
        ...
        with pytest.MonkeyPatch.context() as mp:
            ...
            mp.setattr(TelegramApprovalDecider, "is_pending", classmethod(lambda cls, k: True))
            mp.setattr(
                TelegramApprovalDecider,
                "resolve_global",
                staticmethod(lambda k, approved: order.append("resolved") or True),
            )
    ...
    async def test_a_live_trust_press_really_reaches_the_shared_set(self) -> None:
        ...
        with pytest.MonkeyPatch.context() as mp:
            mp.setattr(TelegramApprovalDecider, "is_pending", classmethod(lambda cls, k: True))
            mp.setattr(TelegramApprovalDecider, "resolve_global", staticmethod(lambda k, a: True))
            await dispatcher.on_callback(_trust_press())
...
class TestApprovalPromptMarkup:
    ...
    async def test_the_tool_name_is_monospaced_not_shown_with_literal_backticks(self) -> None:
        ...
        assert [b["callback_data"] for b in markup["inline_keyboard"][0]] == ["a:r1:1", "a:r1:0"]
