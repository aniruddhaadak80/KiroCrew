...
class TestPortMirror:
    ...
    @pytest.mark.asyncio
    async def test_reconnect_reuses_own_previous_port(self, tmp_path, monkeypatch):
        """An unclean exit leaves local_port set; the instance takes it back."""
        from kiro_crew.instances.registry import InstancesRegistry
        from kiro_crew.instances.ssh_tunnel_manager import TunnelState
        ...
        reg.add(name="CD", ssh_host="cd-1-alias", instance_id="cd-1", remote_port=7900)
        mgr = self._mgr(reg, factory, monkeypatch)
        # Pick a free port at/above the base so the allocator would accept it.
        prior = mgr._allocator.allocate()
        reg.update("cd-1", local_port=prior)  # survivor of a hard kill

        assert (await mgr.connect("cd-1")).state == TunnelState.CONNECTED
        assert captured["lp"] == prior  # own port handed straight back
        assert reg.get("cd-1").local_port == prior
