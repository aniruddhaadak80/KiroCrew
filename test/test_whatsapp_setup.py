...
def cfg_file(tmp_path: Path, monkeypatch) -> Path:
    ...
    Both namespaces need it: the save endpoint holds its own ``config_path``
    binding, and ``KiroCrewConfig.load`` resolves the loader's.
    """
    cp = tmp_path / "config.json"
    monkeypatch.setattr(mod, "config_path", lambda: cp)
