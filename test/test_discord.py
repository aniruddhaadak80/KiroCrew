...
def _cfg(soft: int = 80, default_agent: str = "", dm_scope: str = "per-channel-peer") -> Any:
    return SimpleNamespace(
        ...
        # Empty url exercises the local-port fallback in `!dashboard`, which is
        # the shape a default install actually has.
...
class TestSlashAndReplyCommands:
    ...
    @pytest.mark.asyncio
    async def test_yolo_reports_then_toggles_through_the_shared_grant(self) -> None:
        d, cli, _ = _dispatcher({"u1"})
        await d.handle_message(_inbound("!yolo"))
        assert "YOLO is" in cli.sent[-1][0]
        ...
        with (
            ...
            so.return_value.activate.return_value = mock.Mock(active=True)
            await d.handle_message(_inbound("!yolo on"))
            # Drives the PROCESS-WIDE grant, so a grant taken here is the same
            # one the dashboard and every other channel sees.
            so.return_value.activate.assert_called_once_with("discord")
        assert "YOLO ON" in cli.sent[-1][0]

    @pytest.mark.asyncio
    async def test_yolo_over_slash_passes_its_option_to_the_same_handler(self) -> None:
        d, cli, _ = _dispatcher({"u1"})
        with (
            mock.patch.object(td_mod, "safety_override") as so,
            mock.patch.object(td_mod, "describe_grant_lifetime", return_value="30m"),
            ...
            await d.on_interaction(self._cmd("yolo", {"action": "off"}))
            so.return_value.deactivate.assert_called_once_with("discord")
        assert "YOLO OFF" in cli.responses[-1][1]

    @pytest.mark.asyncio
    async def test_dashboard_refuses_outside_a_dm(self) -> None:
        """A presigned link is a bearer credential for the whole dashboard, and
        an approved guild thread is readable by everyone who can view it."""
        d, cli, _ = _dispatcher({"u1"}, allowed_threads={"t1"})
        cli.thread_channels.add("t1")
        await d.handle_message(_inbound("!dashboard", thread_id="t1", conversation_id="t1"))
        assert "direct message" in cli.sent[-1][0]
        # No token was minted at all, not merely hidden.
        assert "token=" not in cli.sent[-1][0]

    @pytest.mark.asyncio
    async def test_dashboard_mints_a_link_in_a_dm_and_audits_it(self) -> None:
        d, cli, _ = _dispatcher({"u1"})
        with (
            mock.patch("kiro_crew.dashboard.token_auth.generate_token", return_value="TKN"),
            mock.patch.object(td_mod, "sel") as sel_mock,
        ):
            await d.handle_message(_inbound("!dashboard 2h"))
        body = cli.sent[-1][0]
        assert "token=TKN" in body and "2h" in body
        # Credential issuance MUST be audited.
        ops = [
            c.kwargs.get("operation") for c in sel_mock.return_value.log_api_access.call_args_list
        ]
        assert "discord.dashboard_token" in ops
...
class TestCommandSurfaceParity:
    ...
    @pytest.mark.asyncio
    async def test_every_declared_yolo_choice_actually_drives_the_grant(self) -> None:
        """A declared choice value must be one the HANDLER acts on, not merely
        one the parser round-trips.

        Asserting the parser alone is not enough: it happily returns any token,
        and a value the handler does not recognize falls through to the
        report-only branch, so tapping "renew" would silently print status
        instead of renewing. The mutator called is the observable that
        distinguishes them.
        """
        payload = {row["name"]: row for row in application_command_payload()}
        mutators = {"on": "activate", "off": "deactivate", "renew": "renew"}
        # Read the DECLARED option name from the payload rather than assuming
        # it, so the test exercises what Discord will actually send back.
        option = payload["yolo"]["options"][0]
        for choice in option["choices"]:
            value = choice["value"]
            assert value in mutators, f"undeclared yolo action {value!r}"
            d, _cli, _sess = _dispatcher({"u1"})
            with (
                mock.patch.object(td_mod, "safety_override") as so,
                mock.patch.object(td_mod, "describe_grant_lifetime", return_value="30m"),
            ):
                so.return_value.is_active.return_value = value != "on"
                so.return_value.activate.return_value = mock.Mock(active=True)
                so.return_value.renew.return_value = mock.Mock(renewed=True)
                await d.on_interaction(
                    DiscordInteraction(
                        interaction_id="i1",
                        interaction_token="tok",
                ...
                called = getattr(so.return_value, mutators[value])
                assert called.called, f"{value!r} did not reach {mutators[value]}()"

    def test_the_dashboard_ttl_option_survives_the_round_trip(self) -> None:
        assert parse_command("!dashboard 2h") == "dashboard"
        assert parse_dashboard_ttl("!dashboard 2h") == 7200
...
class TestOptionChoiceIsNeverACommand:
    ...
    async def test_a_model_authored_option_label_cannot_execute_a_command(self) -> None:
        ...
        the user picked. Interpreting it would put `!yolo on` one tap from the
        process-wide auto-approve grant."""
        d, cli, sess = _dispatcher({"u1"})
        with (
            mock.patch.object(td_mod, "safety_override") as so,
            mock.patch.object(td_mod, "describe_grant_lifetime", return_value="30m"),
        ):
            # A bare MagicMock reads TRUTHY, which would send `!yolo on` down the
            # "already ON" branch and let the test pass without ever proving the
            # grant was untouched.
            so.return_value.is_active.return_value = False
            so.return_value.activate.return_value = mock.Mock(active=True)
            await d.on_interaction(
            ...
            so.return_value.activate.assert_not_called()
        # The observable difference: a command RETURNS before the session is
        # acquired, so a turn having run is what proves the label was treated as
        # chat text. Asserting on the posted text alone cannot distinguish them,
        # because this path echoes the chosen label either way.
...
class TestOperatorCommandsAreOwnerOnly:
    """`!yolo` and `!dashboard` act on the OPERATOR's authority, not the caller's.

    `allowed_user_ids` answers "may drive the agent", which is a different claim:
    one grants auto-approve for every tool the gateway will run, the other mints a
    login link to the whole dashboard. With several approved users there is no
    unambiguous owner, so both refuse.
    """

    @pytest.mark.asyncio
    @pytest.mark.parametrize("cmd", ["yolo on", "dashboard"])
    async def test_an_ambiguous_allowlist_refuses_both(self, cmd: str) -> None:
        d, cli, _sess = _dispatcher({"u1", "u2"})
        with (
            mock.patch.object(td_mod, "safety_override") as so,
            mock.patch("kiro_crew.dashboard.token_auth.generate_token") as mint,
            mock.patch.object(td_mod, "describe_grant_lifetime", return_value="30m"),
        ):
            await d.handle_message(_inbound(f"!{cmd}"))
            so.return_value.activate.assert_not_called()
            mint.assert_not_called()
        assert "owner-only" in cli.sent[-1][0]

    @pytest.mark.asyncio
    async def test_a_single_allowlisted_user_is_the_owner(self) -> None:
        d, cli, _sess = _dispatcher({"u1"})
        with (
            mock.patch.object(td_mod, "safety_override") as so,
            mock.patch.object(td_mod, "describe_grant_lifetime", return_value="30m"),
    ...
    async def test_the_refusal_is_audited(self) -> None:
        d, _cli, _sess = _dispatcher({"u1", "u2"})
        with mock.patch.object(td_mod, "sel") as sel_mock:
            await d.handle_message(_inbound("!yolo on"))
        calls = sel_mock.return_value.log_api_access.call_args_list
        assert any(
            c.kwargs.get("operation") == "discord.yolo" and c.kwargs.get("outcome") == "denied"
            for c in calls
        )
