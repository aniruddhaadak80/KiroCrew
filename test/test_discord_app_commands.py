...
from kiro_crew.dashboard.token_auth import MAX_SESSION_TTL_SECS
...
# ── parse_command: the four added commands ──
...
class TestAddedCommands:
    ...
    def test_case_and_whitespace(self) -> None:
        ...
        assert parse_command("\t!Dashboard 30M") == "dashboard"
    ...
    def test_near_misses_are_not_commands(self) -> None:
        ...
        assert parse_command("dashboard") is None
        assert parse_command("!dashboards") is None
        assert parse_command("!statuses") is None
        assert parse_command("!yolomode") is None
...
class TestParseDashboardTtl:
    def test_default_without_a_duration(self) -> None:
        assert parse_dashboard_ttl("!dashboard") == 3600
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("!dashboard 2h", 7200),
            ("!dashboard 30m", 1800),
            ("/dashboard 45M", 2700),  # the token is lowercased before parsing
            ("!dashboard 1h", 3600),
            # The duration is the token after the command, whatever follows it.
            ("!dashboard 2h thanks", 7200),
        ],
    )
    def test_valid_durations(self, text: str, expected: int) -> None:
        assert parse_dashboard_ttl(text) == expected

    @pytest.mark.parametrize(
        "text",
        ["!dashboard soon", "!dashboard 2", "!dashboard h", "!dashboard 2 hours", "!dashboard -3h"],
    )
    def test_garbage_falls_back_to_the_default(self, text: str) -> None:
        assert parse_dashboard_ttl(text) == 3600

    def test_ttl_is_read_from_the_token_after_the_command(self) -> None:
        # Discord's form spends no token on a subcommand, so the duration sits at
        # index 1; reading Telegram's index 2 would silently ignore every TTL.
        assert parse_dashboard_ttl("!dashboard 2h") != parse_dashboard_ttl("!dashboard")

    def test_capped_at_the_token_ceiling(self) -> None:
        assert parse_dashboard_ttl("!dashboard 999h") == MAX_SESSION_TTL_SECS
...
class TestFormatTtl:
    @pytest.mark.parametrize(
        "secs,expected",
        [
            (5400, "1h 30m"),  # both components, never truncated to "1h"
            (7200, "2h"),
            (3600, "1h"),
            (1800, "30m"),
            (300, "5m"),
    ...
    def test_rendering(self, secs: int, expected: str) -> None:
        assert format_ttl(secs) == expected
...
class TestApplicationCommandPayload:
    ...
    def test_options_only_where_an_argument_is_taken(self) -> None:
        rows = _rows_by_name()
        assert {name for name, row in rows.items() if row.get("options")} == {
            "yolo",
            "sessions",
            "dashboard",
        }

    @pytest.mark.parametrize("name,option", [("sessions", "query"), ("dashboard", "ttl")])
    ...
    def test_yolo_offers_the_three_actions_as_choices(self) -> None:
        (opt,) = _rows_by_name()["yolo"]["options"]
        assert opt["name"] == "action"
        assert opt["required"] is False
        assert [c["value"] for c in opt["choices"]] == ["on", "off", "renew"]
        assert [c["name"] for c in opt["choices"]] == ["on", "off", "renew"]
    ...
    def test_payload_rows_are_copies(self) -> None:
        ...
        first = _rows_by_name()["yolo"]["options"][0]
        first["choices"].append({"name": "maybe", "value": "maybe"})
        first["choices"][0]["value"] = "clobbered"
...
class TestBuildHelpText:
    ...
    def test_shows_argument_placeholders_from_the_option_table(self) -> None:
        card = build_help_text()
        assert "`!yolo [action]`" in card
        assert "`!sessions [query]`" in card
        assert "`!dashboard [ttl]`" in card
